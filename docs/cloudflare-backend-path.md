...
- Media refresh: `src/main.js` calls `enrichmentService.refreshMedia()`, which calls `POST /api/places/:id/media/refresh` first. The Worker now checks reviewed D1 images, curated/client media, then server-side Wikimedia Commons/Wikidata lookup with D1 `place_images` persistence. Passing `refresh=1` or `{ "force": true }` bypasses stored D1 images for a live recheck, then falls back to stored media if the live pass is empty. The local Commons/Openverse aggregator remains as fallback when the Worker only has designed fallback media.
